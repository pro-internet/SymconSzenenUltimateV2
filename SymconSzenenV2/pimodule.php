...
abstract class PISymconModule extends IPSModule {
    ...
    protected function setVariableTemp ($id, $val, $seconds = 1) {
        ...
        if ($this->doesExist($id)) {
            ...
            $script = $this->checkScript("TimerEnd", "<?php if (IPS_HasChildren(\$_IPS['SELF'])) { foreach (IPS_GetChildrenIDs(\$_IPS['SELF']) as \$child) { IPS_DeleteEvent(\$child); } } SetValue($id, $val); IPS_DeleteScript(\$_IPS['SELF'], true); ?>", false, true, 1000, $id);
